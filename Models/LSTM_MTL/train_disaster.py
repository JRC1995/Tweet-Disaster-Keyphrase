...
Hyperparameters = [window_size, hidden_size, learning_rate, momentum, decay,
                   ...
                   alpha, grad_max_norm, model_name]

with open('JRNN_SOFTMAX_SAVED_MODEL/test_hyperparameters', 'wb') as fp:
...
with tf.Session() as sess:  # Start Tensorflow Session
    ...
    for epoch in range(0, epochs):
        ...
        train_batches_sen, _, train_batches_labels_1, train_batches_labels_2,\
            train_batches_true_seq_lens = bucket_and_batch.bucket_and_batch(
                train_tweets_window, train_tweets, train_labels_1, train_labels_2,
                vocab2idx, batch_size, labels2idx)
        ...
        for i in range(0, len(train_batches_sen)):
            ...
            # break
        val_batches_sen, _, val_batches_labels_1, val_batches_labels_2,\
            val_batches_true_seq_lens = \
            bucket_and_batch.bucket_and_batch(
                val_tweets_window, val_tweets, val_labels_1, val_labels_2,
                vocab2idx, batch_size, labels2idx)
        ...
        if (math.fabs(val_F1-best_val_F1) <= e):

            if best_val_F1_loss >= avg_val_loss:

                impatience = 0

                best_val_F1_loss = avg_val_loss
                best_val_F1 = val_F1
                flag = 1
