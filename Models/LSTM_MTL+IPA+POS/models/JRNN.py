...
class Joint_Encoders:
    ...
    def __init__(self, sentences, pos, ipa, phono, elmo_dims, labels_1, labels_2,
    ...
    def initiate_graph(self):
        ...
        self.D = 3*len(self.word_embeddings[0]) + 64 + 128
        ...
        embd_sentences = tf.reshape(
            embd_sentences, [self.N, self.S, 3*len(self.word_embeddings[0])])
        pos_embd = tf.reshape(pos_embd, [self.N, self.S, 64])

        ultimate_represent = tf.concat(
            [embd_sentences, pos_embd, ipa_phono_embd], axis=-1)  # , ipa_embd
