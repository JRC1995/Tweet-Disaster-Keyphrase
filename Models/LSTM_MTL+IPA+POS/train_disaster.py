...
with tf.Session(config=config) as sess:  # Start Tensorflow Session
    ...
    for epoch in range(0, epochs):
        ...
        train_batches_sen, train_batches_string, train_batches_pos, \
            train_batches_ipa, train_batches_phono, train_batches_labels_1, train_batches_labels_2,\
            train_batches_true_seq_lens = bucket_and_batch.bucket_and_batch(
                train_tweets_window, train_tweets, train_pos, train_ipa, train_phono, train_labels_1, train_labels_2,
                vocab2idx, batch_size, labels_set)
        # print(train_batches_phono[0][1])
        ...
        for i in range(0, len(train_batches_sen)):
            ...
            cost, prediction, acc, _, _ = sess.run([model.loss,
                                                   ...
                                                   feed_dict={tf_sentences: train_batches_sen[j],
                                                              tf_pos: train_batches_pos[j],
                                                              tf_ipa: train_batches_ipa[j],
                                                              tf_phono: train_batches_phono[j],
            ...
            # break
        val_batches_sen, val_batches_string, val_batches_pos, \
            val_batches_ipa, val_batches_phono, val_batches_labels_1, val_batches_labels_2,\
            val_batches_true_seq_lens = bucket_and_batch.bucket_and_batch(
                val_tweets_window, val_tweets, val_pos, val_ipa, val_phono, val_labels_1, val_labels_2,
                vocab2idx, batch_size, labels_set)
        ...
        for i in range(0, len(val_batches_sen)):
            ...
            cost, prediction, acc, = sess.run([model.loss,
                                              ...
                                              feed_dict={tf_sentences: val_batches_sen[i],
                                                         tf_pos: val_batches_pos[i],
                                                         tf_ipa: val_batches_ipa[i],
                                                         tf_phono: val_batches_phono[i],
